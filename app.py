...
with aba_alocacao:
    if not df_custodia_atual.empty:
        ...
        st.info("ℹ️ Nenhum dado de custódia disponível para gerar o raio-x de alocação.")import streamlit as st
import pandas as pd
import numpy as np
import json
import io
...
with aba_alocacao:
    if not df_custodia_atual.empty:
        ...
        st.info("ℹ️ Nenhum dado de custódia disponível para gerar o raio-x de alocação.")        if mov == 'Compra':
            carteira[ticker]['quantidade'] += qtd
            carteira[ticker]['custo_total'] += valor
        elif mov == 'Venda':
            if carteira[ticker]['quantidade'] > 0:
